-- 1. Miremos las tablas disponibles
SELECT table_name 
...
select * from dafbustosus_coderhouse.clima;
...
-- verificando
select * from dafbustosus_coderhouse.desastres;
...
select * from dafbustosus_coderhouse.muertes;
...
-- verificando
select * from dafbustosus_coderhouse.DESASTRES_FINAL;

-- 6. Creando procedimiento

DELETE FROM dafbustosus_coderhouse.DESASTRES_FINAL;
...
FROM dafbustosus_coderhouse.clima as c
JOIN dafbustosus_coderhouse.desastres as d ON c.año =d.año
JOIN dafbustosus_coderhouse.muertes as m ON c.año = m.año

-- Ahora el select completo
SELECT x.Cuatrenio, AVG(x.Temperatura) AS Temp_AVG, 
...
FROM dafbustosus_coderhouse.clima as c
JOIN dafbustosus_coderhouse.desastres as d ON c.año =d.año
JOIN dafbustosus_coderhouse.muertes as m ON c.año = m.año) x
GROUP BY x.Cuatrenio


-- Bien ahora el insert
INSERT INTO dafbustosus_coderhouse.DESASTRES_FINAL
SELECT x.Cuatrenio, AVG(x.Temperatura) AS Temp_AVG, 
AVG(x.NivelOxigeno) AS Oxi_AVG,
SUM(x.Tsunamis) AS T_Tsunamis,
SUM(x.OlasCalor) AS T_OlasCalor,
...
drop table if exists dafbustosus_coderhouse.clima;
drop table if exists dafbustosus_coderhouse.muertes;
drop table if exists dafbustosus_coderhouse.desastres;
drop table if exists dafbustosus_coderhouse.desastres_final;
